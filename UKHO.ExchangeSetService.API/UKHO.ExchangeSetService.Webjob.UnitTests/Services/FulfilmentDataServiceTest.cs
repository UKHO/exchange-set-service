...
namespace UKHO.ExchangeSetService.Webjob.UnitTests.Services
{
    ...
    public class FulfilmentDataServiceTest
    {
        ...
        public async Task WhenValidMessageQueueTrigger_ThenReturnsStandardAndAioExchangeSetCreatedSuccessfully()
        {
            ...
            && call.GetArgument<IEnumerable<KeyValuePair<string, object>>>(2)!.ToDictionary(c => c.Key, c => c.Value)["{OriginalFormat}"].ToString() == "Create aio exchange set product file request for and BatchId:{BatchId} and _X-Correlation-ID:{CorrelationId}").MustHaveHappenedOnceExactly();
        ...
        [Test]
        public async Task WhenInValidMessageQueueTrigger_ThenReturnsLargeMediaAndAioExchangeSetIsNotCreated()
        {
            string filePath = @"D:\\Downloads";
            var b1 = A.Fake<IDirectoryInfo>();
            var b2 = A.Fake<IDirectoryInfo>();
