...
namespace UKHO.ExchangeSetService.API.Controllers
{
    ...
    public class EssWebhookController : BaseController<EssWebhookController>
    {
        ...
        {
            Logger.LogInformation(EventIds.ESSInvalidateAndInsertCacheEventStart.ToEventId(), "ESS Invalidate and Insert Cache Event started for _X-Correlation-ID:{correlationId}", GetCurrentCorrelationId());
            ...
            if (azureAdB2CHelper.IsAzureB2CUser(azureAdB2C, GetCurrentCorrelationId()))
            {
                ...
                Logger.LogInformation(EventIds.ESSInvalidateAndInsertCacheEventCompleted.ToEventId(), "ESS Invalidate and Insert Cache Event completed as Azure AD Authentication failed with OK response and _X-Correlation-ID:{correlationId}", GetCurrentCorrelationId());
                return GetCacheResponse();
            }
            ...
            Logger.LogInformation(EventIds.ESSInvalidateAndInsertCacheEventStart.ToEventId(), "Enterprise Event data deserialized in ESS and Data:{data} and _X-Correlation-ID:{correlationId}", JsonConvert.SerializeObject(data), GetCurrentCorrelationId());
            ...
            if (!validationResult.IsValid)
            {
                ...
                Logger.LogInformation(EventIds.ESSInvalidateAndInsertCacheEventCompleted.ToEventId(), "ESS Invalidate and Insert Cache Event completed for ProductName:{productName} as required data was missing in payload with OK response and _X-Correlation-ID:{correlationId}", productName, GetCurrentCorrelationId());
                return GetCacheResponse();
            }

            await essWebhookService.InvalidateAndInsertCacheData(data, GetCurrentCorrelationId());

            Logger.LogInformation(EventIds.ESSInvalidateAndInsertCacheEventCompleted.ToEventId(), "ESS Invalidate and Insert Cache Event completed for ProductName:{productName} of BusinessUnit:{businessUnit} with OK response and _X-Correlation-ID:{correlationId}", productName, data.BusinessUnit, GetCurrentCorrelationId());
