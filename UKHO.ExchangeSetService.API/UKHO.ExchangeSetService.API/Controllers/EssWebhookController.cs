...
namespace UKHO.ExchangeSetService.API.Controllers
{
    ...
    public class EssWebhookController : BaseController<EssWebhookController>
    {
        ...
        {
            Logger.LogInformation(EventIds.ESSInsertCacheDataEventStart.ToEventId(), "ESS Insert Cache Data Event started for _X-Correlation-ID:{correlationId}", GetCurrentCorrelationId());
            ...
            if (azureAdB2CHelper.IsAzureB2CUser(azureAdB2C, GetCurrentCorrelationId()))
            {
                ...
                Logger.LogInformation(EventIds.ESSInsertCacheDataEventCompleted.ToEventId(), "ESS Insert Cache Data Event completed as Azure AD Authentication failed with OK response and _X-Correlation-ID:{correlationId}", GetCurrentCorrelationId());
                return GetCacheResponse();
            }
            ...
            Logger.LogInformation(EventIds.ESSInsertCacheDataEventStart.ToEventId(), "Enterprise Event data deserialized in ESS and Data:{data} and _X-Correlation-ID:{correlationId}", JsonConvert.SerializeObject(data), GetCurrentCorrelationId());
            ...
            {
                Logger.LogInformation(EventIds.ESSInsertCacheDataValidationEvent.ToEventId(), "Required attributes missing in event data from Enterprise event for ESS Insert Cache Event for _X-Correlation-ID:{correlationId}", GetCurrentCorrelationId());
                Logger.LogInformation(EventIds.ESSInsertCacheDataEventCompleted.ToEventId(), "ESS Insert Cache Data Event completed for ProductName:{productName} as required data was missing in payload with OK response and _X-Correlation-ID:{correlationId}", productName, GetCurrentCorrelationId());
                return GetCacheResponse();
            }
