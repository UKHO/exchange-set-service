...
namespace UKHO.ExchangeSetService.API.Controllers
{
    ...
    public class EssWebhookController : BaseController<EssWebhookController>
    {
        ...
        {
            Logger.LogInformation(EventIds.ESSUpsertSearchAndDownloadCacheEventStart.ToEventId(), "ESS Upsert Cache Event started for _X-Correlation-ID:{correlationId}", GetCurrentCorrelationId());
            ...
            {
                Logger.LogInformation(EventIds.ESSB2CUserValidationEvent.ToEventId(), "Event was triggered with invalid Azure AD token from Enterprise event for ESS Upsert Cache Search and Download Event for _X-Correlation-ID:{correlationId}", GetCurrentCorrelationId());
                Logger.LogInformation(EventIds.ESSUpsertSearchAndDownloadCacheEventCompleted.ToEventId(), "ESS Upsert Cache Event completed as Azure AD Authentication failed with OK response and _X-Correlation-ID:{correlationId}", GetCurrentCorrelationId());
                return GetCacheResponse();
            }
            ...
            {
                Logger.LogInformation(EventIds.ESSClearCacheSearchDownloadValidationEvent.ToEventId(), "Required attributes missing in event data from Enterprise event for ESS Upsert Cache Search and Download Event for _X-Correlation-ID:{correlationId}", GetCurrentCorrelationId());
                Logger.LogInformation(EventIds.ESSUpsertSearchAndDownloadCacheEventCompleted.ToEventId(), "ESS Upsert Cache Event completed for ProductName:{productName} as required data was missing in payload with OK response and _X-Correlation-ID:{correlationId}", productName, GetCurrentCorrelationId());
                return GetCacheResponse();
            }

            await essWebhookService.DeleteSearchAndDownloadCacheData(data, GetCurrentCorrelationId());
            await essWebhookService.InsertSearchAndDownloadCacheData(data, payloadJson, GetCurrentCorrelationId());

            Logger.LogInformation(EventIds.ESSUpsertSearchAndDownloadCacheEventCompleted.ToEventId(), "ESS Upsert Cache Event completed for ProductName:{productName} of BusinessUnit:{businessUnit} with OK response and _X-Correlation-ID:{correlationId}", productName, data.BusinessUnit, GetCurrentCorrelationId());
