    ...
    public class ProductDataValidator : AbstractValidator<ProductDataSinceDateTimeRequest>, IProductDataValidator
    {
        public ProductDataValidator()
        {
            RuleFor(x => x.SinceDateTime)
                .Must(x => x.IsValidRfc1123Format(out DateTime sinceDateTime)).WithMessage("Provided since date time is either invalid or invalid format, the valid formats are 'RFC1123 formats'.")
                .NotEmpty()
                .NotNull()
                .Must(IsValidDate).WithMessage("Since date time cannot be a future date.")
        ...
        private bool IsValidDate(string date)
        {
            try
            {
                return Convert.ToDateTime(date) <= DateTime.UtcNow;
            }
