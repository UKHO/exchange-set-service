...
namespace UKHO.ExchangeSetService.Common.Helpers
{
    public class FileShareService : IFileShareService
    {
        ...
        public async Task<SearchBatchResponse> GetBatchInfoBasedOnProducts(List<Products> products, string batchId, string correlationId)
        {
            ...
            int queryCount = 0;
            logger.LogInformation(EventIds.FileShareServicePreparingToQueryENCFilesStart.ToEventId(), "File share service request started preparing to querying ENC files for BatchId:{batchId} based on {sbLog} and _X-Correlation-ID:{CorrelationId}", batchId, productWithAttributes.Item2, correlationId);
            do
            {
                queryCount++;
            ...
            logger.LogInformation(EventIds.FileShareServicePreparingToQueryENCFilesCompleted.ToEventId(), "File share service request completed preparing and querying ENC files for BatchId:{batchId} based on {sbLog} and _X-Correlation-ID:{CorrelationId}", batchId, productWithAttributes.Item2, correlationId);
