...
namespace UKHO.ExchangeSetService.Common.Helpers
{
    ...
    public class AzureMessageQueueHelper : IAzureMessageQueueHelper
    {
        ...
        public async Task AddMessage(string batchId, int instanceCount, string storageAccountConnectionString, string message, string correlationId)
        {
            ...
            logger.LogInformation(EventIds.AddMessageInQueueSCSResponseStored.ToEventId(), "Add message in Queue {queue} with Sales catalogue response uri for the {batchId} and _X-Correlation-ID:{CorrelationId}", queue, batchId, correlationId);
        }
    }
}
