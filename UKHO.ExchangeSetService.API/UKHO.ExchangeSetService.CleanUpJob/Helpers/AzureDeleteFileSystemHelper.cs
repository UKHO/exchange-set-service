                    ...
                    if (isValid)
                    {
                        ...
                        if (subFolderDate <= historicDate)
                        {
                            ...
                            foreach (DirectoryInfo dir in di.GetDirectories())
                            {
                                ...
                                await azureBlobStorageClient.DeleteContainerFile(storageAccountConnectionString, containerName, batchId);                               
                                dir.Delete(true);
                                
                                logger.LogInformation(EventIds.DeleteHistoricContainerFile.ToEventId(), "SCS response from the container for BatchId:{BatchId} deleted successfully.", batchId);
                            }
                            di.Delete(true);
                            deleteStatus = true;
                            
                            logger.LogInformation(EventIds.DeleteHistoricFolder.ToEventId(), "Historic folder for DateFolder:{dateFolder} deleted successfully.", dateFolder);
                        }
                    }
                }
                return deleteStatus;
