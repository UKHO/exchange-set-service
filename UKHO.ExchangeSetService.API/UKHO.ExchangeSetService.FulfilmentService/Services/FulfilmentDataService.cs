...
namespace UKHO.ExchangeSetService.FulfilmentService.Services
{
                                ...
                                IExchangeSetBuilder exchangeSetBuilder) : IFulfilmentDataService
    {
        ...
        public async Task<string> CreateLargeExchangeSet(FulfilmentServiceBatch batch)
        {
            DateTime createExchangeSetTaskStartedAt = DateTime.UtcNow;
            //string homeDirectoryPath = configuration["HOME"];
            ...
            if (essItems.Count > 0)
            {
                ...
                isExchangeSetFolderCreated = await exchangeSetBuilder.CreateStandardLargeMediaExchangeSet(batch, response);
            ...
            if (isExchangeSetFolderCreated)
            {
                ...
                Parallel.ForEach(rootDirectories, rootDirectoryFolder =>
                {
                    if (rootDirectoryFolder.Name.StartsWith("M0"))  //Large Media Exchange Set
                    {
                        ...
                        parallelZipUploadTasks.Add(PackageAndUploadLargeMediaExchangeSetZipFileToFileShareService(batch.BatchId, rootDirectoryFolder.ToString(), batch.BatchDirectory, batch.CorrelationId, string.Format(batch.LargeExchangeSetFolderName, dvdNumber.ToString())));
                    }
                    else // AIO
                    {
