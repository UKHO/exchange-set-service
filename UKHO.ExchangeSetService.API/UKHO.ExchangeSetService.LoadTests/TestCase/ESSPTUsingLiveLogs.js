...
export const options = {
    ...
    'hardcoded-executor': {
      ...
      preAllocatedVUs: 180, //worst case scenario i.e. Best Case - 8 vus
      timeUnit: '1m',
      stages: [
        { duration: stageTime, target: testStages[0] },
        { duration: stageTime, target: testStages[1] },
        { duration: stageTime, target: 0 }, //7:3
        { duration: stageTime, target: 0 }, //7:4
        { duration: stageTime, target: 0 }, //7:5
...
export function handleSummary(data) {
  return {
    ["ExecutionSummary/Checker-" + new Date().toISOString().substr(0, 19).replace(/(:|-)/g, "").replace("T", "_") + ".html"]: htmlReport(data),
    stdout: textSummary(data, { indent: " ", enableColors: true }),
    ["ExecutionSummary/Checker-" + new Date().toISOString().substr(0, 19).replace(/(:|-)/g, "").replace("T", "_") + ".json"]: JSON.stringify(data),
  }
}