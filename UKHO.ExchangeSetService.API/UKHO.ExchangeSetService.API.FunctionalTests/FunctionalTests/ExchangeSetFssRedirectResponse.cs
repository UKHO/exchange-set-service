...
namespace UKHO.ExchangeSetService.API.FunctionalTests.FunctionalTests
{
    class ExchangeSetFssRedirectResponse
    {
        ...
        public async Task WhenICallTheApiWithProductIdentifiers_ThenARedirectResponseIsReturned()        {
            ...
            Assert.That((int)response.StatusCode,Is.EqualTo(200), $"Incorrect status code File Download api returned {response.StatusCode} for the url {fssDownLoadUrl}, instead of the expected 200.");
            
            Assert.That(response.Headers.Contains("x-redirect-status"),Is.True);       
        ...
        public async Task WhenICallTheApiWithProductIdentifiers_ThenNoRedirectResponseIsReturned()
        {
            ...
            Assert.That(response.Headers.Contains("x-redirect-status"), Is.False);
        }
