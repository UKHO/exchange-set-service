...
namespace UKHO.ExchangeSetService.API.FunctionalTests.FunctionalTests.AIOEnabled
{
    ...
    class ExchangeSetGenerateFilesForEncAndAioProductIdentifierWhenAioIsEnabled : ObjectStorage
    {
        ...
        public async Task SetupAsync()
        {
            ...
            ApiEssResponse = await ExchangeSetApiClient.GetProductIdentifiersDataAsync(DataHelper.GetProductIdentifiersForEncAndAio(), accessToken: EssJwtToken);
            //Get the BatchId
        ...
        {
            var checkFile = FssBatchHelper.CheckforFileExist(DownloadedFolderPath, Config.AIOConfig.ExchangeSetSerialAioFile);
        ...
        public void WhenICallEssWithAioProductAndAioIsEnabled_ThenLargeMediaZipsShouldNotBeAvailable()
        {
            ...
            foreach (var folderName in LargeExchangeSetFolderName)
            {
