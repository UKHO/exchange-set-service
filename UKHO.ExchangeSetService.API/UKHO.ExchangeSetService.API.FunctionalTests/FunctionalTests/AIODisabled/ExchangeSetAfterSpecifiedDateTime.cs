...
namespace UKHO.ExchangeSetService.API.FunctionalTests.FunctionalTests.AIODisabled
{
    class ExchangeSetAfterSpecifiedDateTime
    {
        ...
        {
            var tamperedEssJwtToken = EssJwtToken.Remove(EssJwtToken.Length - 2);
        ...
        {
            var sinceDatetime = DateTime.Now.AddDays(days).AddHours(1).ToString("ddd, dd MMM yyyy HH':'mm':'ss 'GMT'", CultureInfo.InvariantCulture);
            var apiResponse = await ExchangeSetApiClient.GetExchangeSetBasedOnDateTimeAsync(sinceDatetime, accessToken: EssJwtToken);
