...
resource "azurerm_key_vault" "small_exchange_set_kv" {
  ...
  network_acls {
    ...
    virtual_network_subnet_ids = concat(var.small_exchange_set_subnets, [var.agent_2204_subnet, var.agent_prd_subnet])
  }
...
resource "azurerm_key_vault" "medium_exchange_set_kv" {
  ...
  network_acls {
    ...
    bypass                     = "AzureServices"
    ip_rules                   = var.allowed_ips
    virtual_network_subnet_ids = concat(var.medium_exchange_set_subnets, [var.agent_2204_subnet, var.agent_prd_subnet])
  }
...
resource "azurerm_key_vault" "large_exchange_set_kv" {
  ...
  network_acls {
    ...
    bypass                     = "AzureServices"
    ip_rules                   = var.allowed_ips
    virtual_network_subnet_ids = concat(var.large_exchange_set_subnets, [var.agent_2204_subnet, var.agent_prd_subnet])
  }
