...
resource "azurerm_storage_account" "medium_exchange_set_storage" {
  ...
  network_rules {
    ...
    ip_rules                   = var.allowed_ips
    bypass                     = ["Logging", "Metrics", "AzureServices"]
    virtual_network_subnet_ids = concat(var.medium_exchange_set_subnets,[var.m_spoke_subnet])
  }
...
resource "azurerm_storage_account" "large_exchange_set_storage" {
  ...
  network_rules {
    ...
    ip_rules                   = var.allowed_ips
    bypass                     = ["Logging", "Metrics", "AzureServices"]
    virtual_network_subnet_ids = concat(var.large_exchange_set_subnets,[var.m_spoke_subnet])
  }
