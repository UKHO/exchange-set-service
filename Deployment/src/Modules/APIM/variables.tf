...
locals {
 ...
 #group_name            = local.env_name == "prod" ? "${var.group_name}${var.suffix}" : "${var.group_name} ${var.env_suffix[local.env_name]}${var.suffix}"
  group_name            = contains([var.suffix] , "v2") ? (local.env_name == "prod" ? "${var.group_name} ${var.suffix}" : "${var.group_name} ${var.env_suffix[local.env_name]} ${var.suffix}") : (local.env_name == "prod" ? "${var.group_name}" : "${var.group_name} ${var.env_suffix[local.env_name]}")
 #product_name          = local.env_name == "prod" ? "${var.product_name}${var.suffix}" : "${var.product_name} ${var.env_suffix[local.env_name]}${var.suffix}"
  product_name          = contains([var.suffix] , "v2") ? (local.env_name == "prod" ? "${var.product_name} ${var.suffix}" : "${var.product_name} ${var.env_suffix[local.env_name]} ${var.suffix}") : (local.env_name == "prod" ? "${var.product_name}" : "${var.product_name} ${var.env_suffix[local.env_name]}")
 #ui_product_name       = local.env_name == "prod" ? "${var.ui_product_name}${var.suffix}" : "${var.ui_product_name} ${var.env_suffix[local.env_name]}${var.suffix}"
  ui_product_name       = contains([var.suffix] , "v2") ? (local.env_name == "prod" ? "${var.ui_product_name} ${var.suffix}" : "${var.ui_product_name} ${var.env_suffix[local.env_name]} ${var.suffix}") : (local.env_name == "prod" ? "${var.ui_product_name}" : "${var.ui_product_name} ${var.env_suffix[local.env_name]}")
 #api_name              = local.env_name == "prod" ? "${var.api_name}${var.suffix}" : "${var.api_name} ${var.env_suffix[local.env_name]}${var.suffix}"
  api_name              = contains([var.suffix] , "v2") ? (local.env_name == "prod" ? "${var.api_name} ${var.suffix}" : "${var.api_name} ${var.env_suffix[local.env_name]} ${var.suffix}" ) : (local.env_name == "prod" ? "${var.api_name}" : "${var.api_name} ${var.env_suffix[local.env_name]}")
 #apim_api_path         = local.env_name == "prod" ? "${local.service_name}${var.suffix}" : "${local.service_name}-${local.env_name}-${var.suffix}"
  apim_api_path         = contains([var.suffix] , "v2") ? (local.env_name == "prod" ? "${local.service_name}-${var.suffix}" : "${local.service_name}-${local.env_name}-${var.suffix}") : (local.env_name == "prod" ? "${local.service_name}" : "${local.service_name}-${local.env_name}")
